...
async function start() {
	...
	mTogglebar(['jitter'], Flags, flag_toggle, 'dTable', { padding: 0, gap: 10 }); 
...
function flag_toggle(name, val, di) {
	console.log('clicked', name, val); //,x); //return;
	Flags[name] = val;
...
function go_draw() {
	...
	//console.log('drawing');
	if (!TREE.jittering) hasChanged = false;
	cClear(cv, cx);
