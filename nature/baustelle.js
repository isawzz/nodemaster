...
function sentence2tree() {
	...
	for (var i = 0; i < sentence.length; i++) {
		...
		if (ch == 'F' || ch == 'A' || ch == 'B') {
			...
			b = create_branchL(b, angle, len, rColor()); lookupAddToList(C.items, ['branch'], b);
			b.id = id++;
		} else if (ch == '+') {
			angle += toRadian(20); //damag; //da=damag; //angle = b.angle - da; //toRadian(25); //rotate(angle);
		} else if (ch == '-') {
			angle -= toRadian(17); //damag; //da=-damag; //angle = b.angle + da; //toRadian(25); //da/2; //rotate(-angle);
		} else if (ch == '[') {
			stack.push(b);
