...
class Speaker {
	...
	initVoices() {
		...
		if (isdef(mBy('output'))) mBy('output').innerHTML = this.voices.map(x => x.name); //else console.log('voices:', this.voices.map(x => x.name));
	}
	...
	utter(text, r = .5, p = .8, v = MASTERVOLUME, voicekey, callback = null) {
		...
		u.text = text;// 'Hi <silence msec="2000" /> Flash!'; //text.toLowerCase();
		u.rate = r;
		u.pitch = p;
	...
	findSuitableVoice(text, k) {
		...
		//console.log('this.voices',this.voices,k,voiceName);
